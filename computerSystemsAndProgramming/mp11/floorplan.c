...
/*
Group Members: nvasi2, sunnyc3

Intro:
The functions created include rotate, recut, swap_module, swap_topology, get_total_resource,
is_leaf_node, is_internal_node, is_in_subtree, get_expression, init_slicing_tree, and postfix_traversal. 
The function init_slicing_tree takes a node pointer to the parent of the current recursion and an integer 
variable indicating the index on the module array. At each recursion step, first generates an internal 
node with vertical cutline and a right child node attached by a module pointer (as the property of a 
left-skewed slicing tree). Then creates a left child node and step to the next recursion parented by the 
current internal node. The function get_total_resource takes the root pointer and returns total 
sum of resource of modules(the variable resource in module_t) in the tree. The function is_leaf_node 
takes a node pointer and returns 1 if the node pointer points to a leaf node in the slicing tree, 
or 0 otherwise. The function is_internal_node takes a node pointer and returns 1 if the node pointer 
points to an internal node in the slicing tree, or returns 0 otherwise. The function is_in_subtree takes 
two node pointers to nodes a and b in the slicing tree, and returns 1 if the subtree rooted at the node 
b belongs to a part of the subtree rooted at the node a. The procedure get_expression traverses the 
slicing tree in a postfix order and encodes it into a postfix expression. The postfix expression 
is stored in the given array pointed by expression. The procedure postfix_traversal takes three 
input arguments, a node pointer pointing to a subtree from which the current recursive call 
takes place, an integer pointer indicating the position where the postfix expression needs to 
fill, and a pointer to the expression array. The postfix traversal is performed in a recursive manner. 
A node is processed as long as both its two children nodes are handled. The procedure rotate takes a 
pointer to an arbitrary node in the slicing tree and rotates the corresponding module (if any) by 
90 degree. The procedure recut changes the cutline from either horizontal direction to vertical 
direction or vertical direction to horizontal direction. The procedure swap_module takes two node 
pointers and swaps the corresponding module pointers. The procedure swap_topology swaps the two 
subtrees rooted at two given node pointers.
...
int is_leaf_node(node_t* ptr) {
  ...
  //checks if the node at ptr has a left or right child, if it doesn't then it is a leave node
...
int is_internal_node(node_t* ptr) {
  ...
  //checks if the node at ptr has children, if so than it is an internal node
  if(ptr->left != NULL && ptr->right != NULL){
    return 1;
  }
  return 0;
...
int is_in_subtree(node_t* a, node_t* b) {
  ...
  //base case: if either if the ptrs not longer point to a node
  if(a == NULL || b == NULL) return 0;
  //if they are equal to each other than the subtree is found
  if(a == b){
    return 1;
  }
  //recursive case: checks if the left or right child subtree is equal to b
  //if this is true then the function returns 1
...
void rotate(node_t* ptr) {
  ...
  //if the ptr module does not exist, then the function does not run
  if(ptr->module == NULL) return;
  
  //does a basic swap of width and height
  int swap = 0;
  swap = ptr->module->w;
...
void recut(node_t* ptr) {
  ...
  //if the cutline at the ptr is H then it is changed to V and vice versa
  if(ptr->cutline == 1){
    ptr->cutline = V;
  }else{
    ptr->cutline = H;
...
void swap_module(node_t* a, node_t* b) {
  ...
  //does a simple swap of the addresses pointed to when referencing
  //a->module and b->module
  module_t* swap = a->module;
  a->module = b->module;
  b->module = swap;
}
...
void swap_topology(node_t* a, node_t* b) {
  ...
  //a simple swap takes place between the node->parent of a and b
  node_t* sparent = a->parent;
  a->parent = b->parent;
  b->parent = sparent;

  //if b was the left child then the parent->left now points to a otherwise
  //parent->right points to a
  if(a->parent->left == b) a->parent->left = a;
  else a->parent->right = a;

  //if a was the left child then the parent->left now points to b otherwise
  //parent->right points to b
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  //first the function traverses through the left and right branch
  postfix_traversal(ptr->left, nth, expression);
  postfix_traversal(ptr->right, nth, expression);

  //if the current ptr is not a module, the cutline is transfered to the expression
  if(ptr->module==NULL){
    expression[*nth].cutline = ptr->cutline;
    expression[*nth].module = NULL;
  }
  //otherwise the module is added to the expression
  else{
    expression[*nth].module = ptr->module;
    expression[*nth].cutline = UNDEFINED_CUTLINE;
  }
  //after something is added to the expression, the value at nth is incremented
  *nth = *nth + 1;
...
int get_total_resource(node_t* ptr)
{
  ...
  //if the pointed does not exist, 0 is returned
  if(ptr == NULL) return 0;

  //if the module exists, the current module's resource is added to the total along with 
  //the left and right branch's total resources. 
  if(ptr->module != NULL)
    return get_total_resource(ptr->left) + get_total_resource(ptr->right) + ptr->module->resource;
  //otherwise, the left and right branch's resources are added together
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  //memory for the current node is allocated and the pointer is created, the parent is 
  //assigned
  node_t *clptr = (node_t *)malloc(sizeof(node_t));
  clptr->parent = par;
  
  //if the function has at least one more module to create, then the function is recusively 
  //called on the left branch
  if(n+1 < num_modules){
    clptr->left = init_slicing_tree(clptr, n+1);
  }
  //if the is only one module left to create, then it is the final leaf
  //the final leaf is generated, poulated, and the parent is returned 
  else{
    clptr->left = NULL;
    clptr->right = NULL;
    clptr->cutline = UNDEFINED_CUTLINE;
    clptr->module = &modules[n];
    return clptr;
  }

  //the cutline's module is set to NULL and the cutline is set to V
  clptr->module = NULL;
  clptr->cutline = V;

  //the right branch created and memory is allocated, all of the parts of the branch are 
  //populated according to the slicing tree parameters
  clptr->right = (node_t *)malloc(sizeof(node_t));
  clptr->right->module = &modules[n];
  clptr->right->parent = clptr;
  clptr->right->cutline = UNDEFINED_CUTLINE;
  clptr->right->right = NULL;
  clptr->right->left = NULL;
  
  //the pointer to the created node is returned
  return clptr;
}

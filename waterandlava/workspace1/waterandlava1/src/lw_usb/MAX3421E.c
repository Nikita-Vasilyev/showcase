...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	//write val via SPI
	WriteBuffer[1] = val;
	int code = XSpi_Transfer(&SpiInstance, WriteBuffer, NULL, 2);
	//read return code from SPI peripheral (see Xilinx examples) 
	//if return code != 0 print an error
	if (code != 0) {
		xil_printf("failed single-byte write");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	for (int Count = 1; Count < nbytes+1; Count++) {
		WriteBuffer[Count] = data[Count-1];
	}
	//write reg + 2 via SPI
	WriteBuffer[0] = (reg+2);
	//write data[n] via SPI, where n goes from 0 to nbytes-1
	int code = XSpi_Transfer(&SpiInstance, WriteBuffer, NULL, nbytes+1);
	//read return code from SPI peripheral 
	//if return code != 0 print an error
	if (code != 0) {
		xil_printf("failed multi-byte write");
	}
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	return (data + nbytes);
...
BYTE MAXreg_rd(BYTE reg) {
	...
	WriteBuffer[0] = reg;
	//read val via SPI
	int code = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, 2);
	//read return code from SPI peripheral 
	//if return code != 0 print an error
	if (code != 0) {
		xil_printf("failed single-byte read");
	}
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	BYTE ret = ReadBuffer[1];
	return ret;
}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//read return code from SPI peripheral 
	int code = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, nbytes+1);
	for( int Count = 0; Count < nbytes; Count++){
		data[Count] = ReadBuffer[Count+1];
	}
	//if return code != 0 print an error
	if (code != 0) {
		xil_printf("failed multi-byte read");
	}
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	return (data + nbytes);
